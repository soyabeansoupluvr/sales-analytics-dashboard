...
_REQUIRED_BY_METRIC: Final[dict[str, frozenset[str]]] = {
    "revenue_summary": frozenset({"InvoiceNo", "IsReturn", "IsAdjustment", "Revenue"}),
    "product_metrics": frozenset(
        {"StockCode", "Description", "Quantity", "IsReturn", "IsAdjustment", "Revenue"}
    ),
    "time_series": frozenset({"InvoiceNo", "InvoiceDate", "IsReturn", "IsAdjustment", "Revenue"}),
    "country_metrics": frozenset({"InvoiceNo", "Country", "IsReturn", "IsAdjustment", "Revenue"}),
    "repeat_rate": frozenset({"InvoiceNo", "CustomerID", "IsReturn", "IsAdjustment"}),
    ...
    *,
    ...
    _audit(
        ...
        details={
            ...
            "stability_score": (None if stability_score is None else round(stability_score, 4)),
    ...
    return pd.DataFrame(
        {
            ...
            "stability_score": pd.Series([score_value] * row_count, dtype="float64"),
            "stability_holdout_size": pd.Series([int(holdout_size)] * row_count, dtype="int64"),
            "stability_threshold": pd.Series([float(threshold)] * row_count, dtype="float64"),
            "stability_flag": pd.Series(
                [None if flag is None else bool(flag)] * row_count,
                dtype="object",
            ),
            "stability_reason": pd.Series([reason_text] * row_count, dtype="object"),
        }
    )
